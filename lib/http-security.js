...
function configuredOrigins() {
  ...
  if (netlifyUrl) allowed.push(netlifyUrl);
  const deployUrl = (process.env.DEPLOY_PRIME_URL || "").replace(/\/$/, "");
  if (deployUrl) allowed.push(deployUrl);
...
function allowOriginHeader(event) {
...
function jsonResponse(statusCode, body, extraHeaders = {}) {
  return {
    ...
    headers: {
      ...
      "Access-Control-Allow-Origin": extraHeaders["Access-Control-Allow-Origin"] || corsOrigin(),
      ...extraHeaders,
    },
...
function corsJson(statusCode, body, methods = "GET, OPTIONS", event = null) {
  return jsonResponse(statusCode, body, {
    "Access-Control-Allow-Origin": allowOriginHeader(event),
...
function checkOrigin(event) {
  ...
  if (isOriginAllowed(origin)) return { allowed: true };
...
function isOriginAllowed(origin) {
  ...
  return configuredOrigins().some((allowed) => normalizeOrigin(allowed) === normalized);
}

function publicSiteUrl(event) {
  const origin = event?.headers?.origin || event?.headers?.Origin || "";
  if (origin && isOriginAllowed(origin)) {
    return normalizeOrigin(origin);
  }
